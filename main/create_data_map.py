...
def make_request_and_edit_query_until_success(query_string):
    retry_limit = len(format.extract_fields_from_query_string(query_string)) - 1 # minus one because we already queried.
...
def main():
    try:
        ...
        for object_id in object_ids:
            ...
            if counts_of_non_null_field_values['hasError']: # if counts fail, the rest of the analysis is useless.
                continue
            else:
            ...
            for field in fields:  # best to do for each field
                ...
                if most_common_values_in_field['records']:
                    ...
                    for i in range(len(fields)):  # 10 because we want to include 'expr0' through 'expr9'
                        key = 'expr' + str(i)
